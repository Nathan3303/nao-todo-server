...
func DeleteTagHandlerV1(ctx *gin.Context) {
	...
	var result *gorm.DB
	if dto.isHardDelete {
		result = core.DB.Unscoped().Where("id = ? and user_id = ?", dto.TagId, userId).Delete(&modules.Tag{})
	} else {
		result = core.DB.Where("id = ? and user_id = ?", dto.TagId, userId).Delete(&modules.Tag{})
	}
